	...
	switch(mode)
	{
		case 'A'://start--启动所有电机
			  //A
				...
				Set_PWM_Compare(&htim8,MOTOR_A_PWM_CHANNEL,2000);
				//B
				Start_Motor(MOTOR_B,(DIRECT_MOTOR)0);
				Set_PWM_Compare(&htim8,MOTOR_B_PWM_CHANNEL,2000);
				//C
				Start_Motor(MOTOR_C,(DIRECT_MOTOR)0);
				Set_PWM_Compare(&htim8,MOTOR_C_PWM_CHANNEL,2000);
				//D
				Start_Motor(MOTOR_D,(DIRECT_MOTOR)0);
				Set_PWM_Compare(&htim8,MOTOR_D_PWM_CHANNEL,2000);
				DEBUG("Start motor\r\n");
				break;
		case 'S'://根据指令启动单个电机
				#if 0
...
void uart_debug(void)
{
	...
	// 	D_enc=Get_Encode_Motor_D();

	// 	sprintf(count_str, "%d", D_enc);

	// 	HAL_UART_Transmit(&huart2, (uint8_t *)count_str, strlen(count_str), HAL_MAX_DELAY);
