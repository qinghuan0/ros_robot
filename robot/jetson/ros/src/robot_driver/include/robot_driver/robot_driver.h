...
class RoboDriver
{
    ...
    bool initRobot();
    void recv_msg();
    void cmd_vel_callback(const geometry_msgs::Twist::ConstPtr& msg);//订阅cmd_vel话题的触发的回调函数
    ...
    void distrubute_data(uint8_t msg_type, uint8_t* buffer_data);//接收数据后的分发，然后进行数据的提取
    ...
    tr2_ros::TransformBroadcaster br_;
